...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const orders = await db.select().from(purchaseOrders).where(eq(purchaseOrders.supplierId, req.params.id)).orderBy(desc(purchaseOrders.createdAt)).limit(10);
...
router.put("/:id", async (req, res, next) => {
  try {
    const [row] = await db.update(suppliers).set({ ...req.body, updatedAt: new Date() }).where(and(eq(suppliers.id, req.params.id), eq(suppliers.tenantId, req.user!.tenantId))).returning();
    if (!row) throw new AppError(404, "المورد غير موجود");
    success(res, row);
  } catch (err) { next(err); }
});
...
router.delete("/:id", async (req, res, next) => {
  try {
    const [row] = await db.update(suppliers).set({ deletedAt: new Date(), isActive: false, updatedAt: new Date() }).where(and(eq(suppliers.id, req.params.id), eq(suppliers.tenantId, req.user!.tenantId))).returning();
    if (!row) throw new AppError(404, "المورد غير موجود");
