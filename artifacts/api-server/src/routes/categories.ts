...
router.put("/:id", async (req, res, next) => {
  try {
    const [row] = await db.update(categories).set({ ...req.body, updatedAt: new Date() }).where(and(eq(categories.id, req.params.id), eq(categories.tenantId, req.user!.tenantId))).returning();
...
router.delete("/:id", async (req, res, next) => {
  try {
    const [row] = await db.update(categories).set({ isActive: false, updatedAt: new Date() }).where(and(eq(categories.id, req.params.id), eq(categories.tenantId, req.user!.tenantId))).returning();
    if (!row) throw new AppError(404, "الفئة غير موجودة");
